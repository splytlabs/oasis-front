import type { TabName } from './nft-search-panel';
...
export default function NftSearchPanelCategory({name}: NftSearchPanelCategoryProps) {
  ...
  return (
    <p className={tw(style)}>
      {name}
      {filterCount !== 0 && <span className='count'>
          {filterCount}
        </span>}
    </p>
  );
